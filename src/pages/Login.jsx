...
import { 
  User, Mail, Lock, ShoppingBag, ArrowRight, 
  Eye, EyeOff, Sparkles, CheckCircle, Phone, 
  MapPin, AlertCircle, XCircle 
} from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    const newUser = {
      ...
      avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.name)}&size=128&background=F59E0B&color=fff&bold=true`
    };

    // Auto-login after registration
    login(newUser, 'fake-jwt-token-register-12345');
  ...
  return (
    <div className="min-h-[calc(100vh-120px)] flex items-center justify-center bg-gradient-to-br from-gray-50 via-white to-amber-50/30 px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="relative max-w-lg w-full space-y-6 bg-white/80 backdrop-blur-sm p-8 rounded-3xl border border-amber-100/50 shadow-2xl shadow-amber-500/5">
        ...
        <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <div className="relative group">
              <input
                ...
                onChange={handleChange}
              />
              <Phone className={`absolute left-3.5 top-1/2 -translate-y-1/2 h-4.5 w-4.5 transition ${
                errors.phone ? 'text-red-400' : 'text-gray-400 group-focus-within:text-amber-500'
              }`} />
              {errors.phone && (
                <div className="absolute right-3.5 top-1/2 -translate-y-1/2">
                  <AlertCircle className="h-4.5 w-4.5 text-red-500" />
          ...
          <div>
            ...
            <p className="mt-1.5 text-xs text-gray-400 flex items-center gap-1">
              <Sparkles className="h-3 w-3" /> Minimum 5 characters
            </p>
          </div>
          ...
          <div>
            ...
            <div className="relative group">
              <input
                ...
                onChange={handleChange}
              />
              <MapPin className={`absolute left-3.5 top-1/2 -translate-y-1/2 h-4.5 w-4.5 transition ${
                errors.address ? 'text-red-400' : 'text-gray-400 group-focus-within:text-amber-500'
              }`} />
              {errors.address && (
                <div className="absolute right-3.5 top-1/2 -translate-y-1/2">
                  <AlertCircle className="h-4.5 w-4.5 text-red-500" />
        ...
        <div className="text-xs text-gray-400 text-center space-y-1">
          <p className="flex items-center justify-center gap-2">
            <CheckCircle className={`h-3 w-3 ${formData.password.length >= 5 ? 'text-emerald-500' : 'text-gray-300'}`} />
