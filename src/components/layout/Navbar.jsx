  ...
  return (
    <nav className={`sticky top-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center px-2 sm:px-4 lg:px-8 h-12 sm:h-14 md:h-16 lg:h-20">
        ...
        {/* Links - Desktop */}
        <div className="hidden md:flex items-center gap-1.5 sm:gap-4 md:gap-6 text-xs sm:text-sm font-medium text-gray-600">
          {navLinks.map((link) => (
            <Link 
              key={link.path}
              to={link.path} 
              className="flex items-center gap-1 hover:text-amber-500 transition group"
            >
              <link.icon className="h-3.5 sm:h-4 w-3.5 sm:w-4 group-hover:scale-110 transition" /> 
              <span className="group-hover:translate-x-0.5 transition hidden sm:inline">{link.label}</span>
            </Link>
          ))}
        </div>
